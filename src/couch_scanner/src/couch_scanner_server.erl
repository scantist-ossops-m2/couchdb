...
init(_Args) ->
    ...
    St = #st{rlimiter = couch_scanner_rate_limiter:new()},
...
handle_info(?TIMEOUT, #st{} = St) ->
    ...
    St2 = start_stop(St1),
    St3 = refill_rate_limiter_buckets(St2),
    St4 = schedule_timeout(St3),
    {noreply, St4};
...
refill_rate_limiter_buckets(#st{rlimiter = RLimiter} = St) ->
    RLimiter1 = couch_scanner_rate_limiter:refill(RLimiter, interval_sec()),
    St#st{rlimiter = RLimiter1}.
...
start_plugin(Id, #st{} = St) ->
    #st{pids = Pids, scheduling = Scheduling, rlimiter = RLimiter} = St,
    Sched = maps:get(Id, Scheduling, #sched{}),
    Now = tsec(),
    case Now >= Sched#sched.reschedule of
        true ->
            couch_log:info("~p : starting ~s", [?MODULE, Id]),
            Pids1 = Pids#{Id => couch_scanner_plugin:spawn_link(Id, RLimiter)},
            Sched1 = Sched#sched{
                start_time = Now,
                reschedule = 0
            },
...
penalize(Now, #sched{error_count = ErrorCount} = Sched) ->
    Penalty = ?PENALTY_BASE_SEC * (1 bsl (ErrorCount - 1)),
...
interval_sec() ->
    cfg_int("sched_interval_sec", ?SCHED_INTERVAL_SEC).
